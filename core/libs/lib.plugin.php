...
class plugin {
	...
	public static function getPluginPath($class_name) {
		return str_replace(['core\\plugins','\\'], ['core.plugins','/'], $class_name).'/';
	}
	...
	public function render($view, $params = [], $buffering = true) {
		...
		$viewFilename = $viewPath.$view.'.php';

		while (!is_file($viewFilename)) {
			$widget_class = get_parent_class($plugin_class);
			if ($widget_class == 'core\\libs\\plugin') {
				trigger_error(debug::_('PLUGIN_RENDER_VIEW_NOT_FOUND', $viewFilename), E_USER_0WARNING);
				return '';
			}
			$viewPath = self::getPluginPath($widget_class).'views/';
			$viewFilename = $viewPath.$view.'.php';
		}
